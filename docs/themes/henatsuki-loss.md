...
- **2026 年度以降**：**第 3 次判断基準** が適用される。対象機種・基準値がさらに強化される見込み

#### 基準負荷率（容量で分かれる）

| 定格容量 | 基準負荷率 | 評価式の銅損項 |
|---|---:|---|
| **500 kV·A 以下** | **40 %** | $W_{c40} = 0.4^2 \times W_{c,full} = 0.16\, W_{c,full}$ |
| **500 kV·A 超** | **50 %** | $W_{c50} = 0.5^2 \times W_{c,full} = 0.25\, W_{c,full}$ |

**判定式**：$\text{省エネ基準達成率}[\%] = \dfrac{\text{基準エネルギー消費効率}}{W_i + W_{c,\text{基準負荷率}}} \times 100$

!!! tip "R04下 問12 の試験上の意味づけ"
    本問の変圧器は **500 kV·A**（境界値）。判断基準の対象である 500 kV·A 以下の枠に入るため、**負荷率 40 % 評価** が適用される。設問 (a) で 1,250 W を $W_i + W_{c40}$ で割って 140 % を出すロジックは、まさに「500 kV·A 以下機の基準負荷率 40 %」というルールを暗黙の前提にしている。容量が 500 kV·A 超だったら 50 % 評価になり、$W_{c50}$（0.25 倍）を使うため答えの数値が変わる。

!!! warning "規制値の改定タイミングに注意"
    トップランナー基準は **施行年で具体的数値が変わる**。本ページの整理は 2026 年時点の運用解釈（第 2 次→第 3 次への移行期）。最新の基準値は経済産業省告示で確認すべき。試験問題は出題時点の規制値を前提とするので、問題文に明示された値（本問なら基準エネルギー消費効率 1,250 W・達成率 140 %）をそのまま使う。
