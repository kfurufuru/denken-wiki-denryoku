...
| 適用落差 | **高落差**（300m以上が多い）・小流量 | **低〜中落差**（フランシス：40〜600m、カプラン：10〜60m） |
| 比速度 Ns | 低い（10〜30） | 高い（フランシス：50〜350、カプラン：200〜900） |
...
### なぜ P = 9.8QHη なのか（物理的導出）

水の位置エネルギーを出発点として導出する。

高さHにある質量mの水の位置エネルギーは `E = mgH`。

これを時間あたりに換算する：
- 流量Q [m³/s] = 毎秒流れる水の体積
- 水の密度 ρ = 1000 kg/m³ なので、毎秒の質量 = ρQ = 1000Q [kg/s]
- 毎秒の位置エネルギー = ρQgH = 1000 × Q × 9.8 × H = 9800QH [W] = 9.8QH [kW]

ここにエネルギー変換の総合効率 η（水車効率 × 発電機効率）を掛けると：

$$P = 9.8 \times Q \times H \times \eta \quad [\mathrm{kW}]$$

!!! note "重要公式：発電出力（必須）"
    $$P = 9.8 \times Q \times H \times \eta \quad [\mathrm{kW}]$$

    - **9.8**：水の比重量 (ρg = 1000 × 9.8 = 9800 N/m³) を kW換算した係数
    - **Q**：流量 [m³/s]
    - **H**：有効落差（総落差 − 損失水頭）[m]
    - **η**：総合効率 = η_水車 × η_発電機
...
!!! note "重要公式：揚水発電（発電・揚水の方向に注意）"
    **発電時**（効率は「掛ける」）：$P_{gen} = 9.8 \times Q \times H \times \eta_{gen}$

    **揚水時**（効率は「割る」）：$P_{pump} = \dfrac{9.8 \times Q \times H_p}{\eta_{pump}}$

    **総合効率**：$\eta_{total} = \eta_{gen} \times \eta_{pump}$（H ≒ H_p の場合）

    揚水時にηで割る理由：損失がある分、余分に電力を投入しなければならないから。

!!! note "重要公式：比速度"
    $$N_s = \frac{N \cdot \sqrt{P}}{H^{5/4}}$$

    - N：回転速度 [min⁻¹]、P：出力 [kW]、H：有効落差 [m]
    - 単位は [m·kW]（回転速度の rpm とは別物。「rpm」と書かないこと）
    - Ns が大きいほど「低落差・大流量」型。ペルトン（低Ns）→ フランシス（中Ns）→ カプラン（高Ns）の順。
...
!!! warning "引っかけパターン：有効落差を使わず総落差をそのまま代入"
    問題文に「総落差 H₀」と「損失水頭 h」が別々に与えられているとき、**P = 9.8QH₀η は誤り**。
    必ず H = H₀ − h を先に計算してから代入すること。

!!! warning "引っかけパターン：揚水時の効率の向き"
    「揚水所要電力 P = 9.8QHη」は誤り。揚水時は損失分だけ余分に電力が必要なので **P = 9.8QH / η**（ηで割る）。発電と揚水で効率の向きが逆になることが最頻出の引っかけ。

!!! warning "引っかけパターン：水車の種類と落差の対応"
    「ペルトン水車は低落差・大流量に適している」→ **誤**。ペルトンは高落差・小流量用（ノズル噴流で使う衝動水車）。低落差・大流量はカプランまたはフランシス水車。

!!! warning "引っかけパターン：調整池式と貯水池式の違い"
    「調整池式は季節間の負荷調整に使われる」→ **誤**。季節間調整は「貯水池式」。調整池式は日〜週単位の調整。
...
> 📚 横断一覧: [分野別一覧（該当分野）](../kakomon/by-field.md#suiryoku) ／ [年度別一覧](../kakomon/by-year.md)
...
> 詳細解説: [電験王 水力カテゴリ](https://denken-ou.com/denryoku/?cat=suiryoku)
...
- [新エネルギー発電](shin-energy.md) — 再エネ普及に伴う揚水発電の系統安定化機能
