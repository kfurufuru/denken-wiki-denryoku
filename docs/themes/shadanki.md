...
| **空気遮断器** | ABB | 圧縮空気 | 騒音大・大型。※ACB（気中遮断器・低圧用、大気中で消弧）とは別物 | 大容量変電所（旧来型） | × 旧式 |
| **ガス遮断器** | GCB | SF₆ガス | 絶縁性能が高い・小型化可能。GCBを内蔵した一体型設備がGIS | 都市型変電所・GIS | ○ 主流 |
...
| GCB | **SF₆ガス** | ○ 主流 | 「GIS は圧縮空気で絶縁」→ 誤（GISはSF₆。ABB と取り違え） |
...
電験王実題で照合した令和の遮断器・断路器・GIS の出題（問番号は年により変動）:

| 年度 | 問 | 出題内容 | 問題タイプ | 難易度 |
|------|---|---------|----------|--------|
| R07上 | 問6 | ガス絶縁開閉装置（GIS）の特徴 | 論説 | ★☆☆☆☆ |
...
!!! note "R08 予測"
    遮断器・断路器・GIS は**問6または問8**に分散して出題される（かつて「問8固定」と言われたが、令和の問8は架空送電が最多）。GIS の絶縁媒体（SF₆）・DS→CB の操作順序・VCB の消弧・断路器の遮断能力のいずれかが穴埋・論説で出る見込み。
