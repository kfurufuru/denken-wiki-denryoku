...
!!! success "🎯 このページで得点できる問題"
    ...
    - **主要継電器の略称・制御器具番号・用途**：OCR(51) / OCGR(51G) / DGR(67G) / UVR(27) / OVR(59) / 87 / 21
...
複数の継電器が同じ CT・VT の二次出力を共有することが多い。**CT 負担**（接続継電器の合計 VA）が定格を超えないよう注意（→ [計器用変成器ページ §4 レイヤー C](keiki-hensei.md#formula-map)）。

---

## 3. 保護継電器の比較表（最重要） {#comparison}

| 継電器 | 略称 | 制御器具番号 | 検出対象 | 動作条件 | 主な用途 |
|-------|------|---------|---------|---------|---------|
...
| **過電圧継電器** | OVR | 59 | 電圧上昇 | 電圧が整定値以上 | 単独運転防止・電圧上昇保護 |
| **地絡過電圧継電器** | OVGR | 64 | 零相電圧（EVT 経由） | 零相電圧が整定値以上 | 非接地系の地絡検出 |
| **差動継電器** | 87 | 87 | 変圧器・発電機内部故障 | 入出力電流の差 | 変圧器・発電機の内部保護 |
| **距離継電器** | 21 | 21 | 線路インピーダンス | インピーダンス整定値以下 | 長距離送電線の保護 |

!!! note "制御器具番号（JEM 1090）は ANSI/IEEE デバイス番号と共通"
    51（過電流）・87（差動）・21（距離）等の番号は、日本では **JEM 1090（日本電機工業会規格）** の制御器具番号で、米国 **ANSI/IEEE C37.2** と共通の体系。試験では番号と日本語名・略称を結びつけて問われる（「JIS番号」ではない点に注意）。

---

## 4. 動作特性（定限時・反限時・差動原理） {#characteristics}
...
DGR の入力は ZCT（零相電流）＋EVT（零相電圧）のセット（→ [計器用変成器 §3](keiki-hensei.md#comparison)）。
...
### 解法パターン①：継電器の略称・制御器具番号・用途を結びつける（R06上問8型）

**典型問題**：「次の保護継電器の名称と 制御器具番号の組合せのうち、誤っているものを選べ」

**判別マトリクス**

| 略称 | 制御器具番号 | 検出対象 | 引っかけ筆頭 |
|---|---|---|---|
...
→ 制御器具番号は ANSI/IEEE C37.2 と共通。**51（過電流）・87（差動）・21（距離）** の 3 つは暗記必須。
...
| 「制御器具番号 87 は差動継電器である」 | **正** | ANSI/IEEE C37.2 と共通の番号体系 |
...
電験王実題で照合した令和の保護継電器の出題（単独論点は少なく、問番号も変動）:

| 年度 | 問 | 出題内容 | 問題タイプ | 難易度 |
|------|---|---------|----------|--------|
| R04上 | 問7 | 過電流継電器（OCR）の限時特性 | 穴埋 | ★★☆☆☆ |
| R01 | 問8 | 保護継電器（OCR）の動作時間 | 計算 | ★★★★★ |

> 詳細解説: [電験王 開閉・保護カテゴリ](https://denken-ou.com/denryoku/?cat=kaiheihogo)

!!! note "R08 予測"
    保護継電器**単独**の出題は少なく（令和ではR04上問7・R01問8）、問番号も固定でない。ただし OCR・DGR・差動・距離の動作原理は配電の地絡保護や変電の論説に**断片的に**織り込まれるため、[自家用受電設備の地絡保護](jikayou-chiraku.md)と合わせた横断学習が有効。R08 では DGR の方向判定／反限時特性／差動原理のいずれかが問われる可能性。
