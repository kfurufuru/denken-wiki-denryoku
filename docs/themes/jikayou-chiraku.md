...
- **PAS（区分開閉器）**：電柱上の引込口に設置。**GR 付 PAS／DGR 付 PAS** とし、構内**地絡**時は GR/DGR が検出して **即時開放**（6.6 kV 非接地系の地絡電流は数〜十数 A と小さく、負荷開閉器の PAS で開閉できる）。一方 **短絡（過電流）** 時は短絡電流が大きく PAS では遮断できないため、**蓄勢（ロック）** して配電用変電所の CB が短絡電流を遮断し線路が **無電圧** になってから開放する。この「地絡＝即時開放／短絡＝蓄勢後の無電圧開放」を担うのが **SOG（蓄勢トリップ付地絡トリップ）** 動作。
...
| 誤った記述 | 正しい記述 |
|---|---|
| 「GR 付 PAS は地絡電流をそのまま遮断する遮断器である」 | **誤**：PAS は負荷開閉器で**遮断器ではない**。地絡は GR 検出で **即時開放**できる（地絡電流が小さいため）が、**短絡電流は遮断できず**、蓄勢して配変 CB の遮断による無電圧後に開放する（SOG 動作） |
...
| 「GR 付 PAS は短絡電流を直接遮断できる」 | **誤** | PAS は開閉器で短絡遮断能力なし。短絡時は蓄勢し配変 CB 遮断後に無電圧開放（地絡は GR 検出で即時開放可） |
...
自家用地絡保護（GR/DGR・もらい事故）は電力科目で**単独の問**としては出題実績が乏しく、配電の「高圧受電設備の保護・非接地系の地絡保護」の中に織り込まれる（電験王実題で照合）:

| 年度 | 問 | 出題内容（地絡保護に関連する部分） | 問題タイプ | 難易度 |
|------|---|---------|----------|--------|
...
> これらは電験王分類では**配電**の問。GR/DGR・ZCT/EVT・もらい事故の因果は、**法規の自家用電気工作物・保安管理**でも問われる横断領域。
>
> 詳細解説: [電験王 配電カテゴリ](https://denken-ou.com/denryoku/?cat=haiden)
