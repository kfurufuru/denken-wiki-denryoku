...
!!! note "電圧降下式の√3と(Rcosθ + Xsinθ)の導出"
    ベクトル図から正確には $\Delta V = \sqrt{(\Delta V_R)^2 + (\Delta V_X)^2}$ だが、実用上は近似式 $\Delta V \approx \sqrt{3}I(R\cos\theta + X\sin\theta)$ が使われる（負荷が遅れ力率の場合）。この近似が成立する条件は「電圧降下が受電端電圧に比べて十分小さい（実用上の送電線では成立する）」こと。$R\cos\theta$ は有効電力成分による降下、$X\sin\theta$ は無効電力成分による降下を表す。
