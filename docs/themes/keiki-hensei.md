...
!!! note "🗺️ 棲み分けマップ"
    **本ページ＝計器用変成器（VT・CT）**：計器用変成器の **総称概念・接続・極性・誤差・残留回路** の全体俯瞰と頻出論点。**5 年で 3〜4 問**（R03 / R05 上 / R06 下／R04 下含む）の継続出題枠で、合格水準の必須テーマ。
...
**手順**
1. 残留回路電流 = 3 I₀（二次値）= 0.6 A
2. 二次零相電流 I₀(二次) = 0.6 / 3 = 0.2 A
3. 変流比 $K_n = 200/5 = 40$
4. 一次零相電流 $I_0 = 0.2 \times 40 = 8$ A
5. 一線地絡電流 $I_g = 3 I_0 = 24$ A

→ 「3 で割って 40 倍して 3 倍する」と覚える（数学的には $K_n$ 倍する＝二次の 3I₀ を一次へ写し取る）。
...
→ **R01〜R07 で 4 問出題（5 年で 3〜4 問・変圧器ハブと同水準の頻出枠）**
...
- [開閉装置・保護継電器](kaihei-hogo.md) — CT・VT 出力を入力とする OCR / DGR / 87 / 21 の整定・動作論
