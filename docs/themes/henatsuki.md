...
!!! tip "現場視点：高調波と鉄損の急増"
    インバータ機器やコンバータが多い負荷では電源電圧に高調波が乗る。**電圧**に第n次高調波（電圧振幅が基本波の $k$ 倍）が乗ると、磁束振幅は $B \propto V/f$ より $k/n$ になる。渦電流損は $\propto (n f B)^2$ なので、その追加分は $(n \times k/n)^2 = k^2$ ——**次数によらず電圧高調波率の2乗**。第5次・電圧10%（$k=0.1$）なら渦電流損の追加は約 $k^2 = 1\%$ 程度で、大きくはない（$(5)^2 \times (0.1)^2 = 0.25$ となるのは「磁束振幅が10%」の場合のみ）。とはいえ多数の高調波が重畳すると無視できず、受変電設備で「LTC変圧器の運転温度が上がってきた」場合は高調波負荷の増加を疑う典型ケース。
...
!!! note "%Z が変圧器ごとに違う理由"
    %Z は変圧器の **設計時** に決まる固有値（鉄心寸法・巻線抵抗・漏れリアクタンスで決定）。一般に **大容量機ほど %Z は大きく**（10〜15%）、小容量機ほど小さい（5〜7%）傾向がある（値は用途・電圧階級・短絡電流制限の設計方針により変動するので、あくまで目安）。これは「並行運転で同じ容量比でも %Z が違うと均等分担できない」典型理由 — 容量が違う2機を並列接続するときは %Z 一致を別途確認する必要がある。
...
!!! note "結線5方式の比較表（必修暗記）"
    ...
    | **Y-Δ** | 星-三角 | あり/なし | **−30°**（二次が遅れ） | 二次Δで循環吸収 | **降圧用**（受電端・変電所側） | 一次中性点接地可能・接地系統と組み合わせやすい |
...
!!! warning "引っかけ②：効率最大 = 全日効率最大 と混同する"
    瞬時効率最大（$P_i = P_c$ の条件）は特定の負荷率で成立する。全日効率は1日24時間の負荷変動を積分した**別概念**で、軽負荷の時間が長いほど鉄損の小さい変圧器が有利（鉄損は24時間かかり続けるため）。
    定義式・解法フロー・計算例は深掘りページに一本化 → [変圧器損失・効率 §2](henatsuki-loss.md#zennichi-flow)
