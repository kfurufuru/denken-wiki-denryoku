<!-- 略語ツールチップ定義 — 電験3種 電力Wiki -->
<!-- pymdownx.snippets の auto_append で全ページに自動適用 -->

*[AC]: 交流 (Alternating Current)
*[DC]: 直流 (Direct Current)
*[kV]: キロボルト — 1000ボルト
*[MVA]: メガボルトアンペア — 皮相電力の単位
*[MW]: メガワット — 有効電力の単位
*[kW]: キロワット — 有効電力の単位
*[kWh]: キロワット時 — 電力量の単位
*[pf]: 力率 (Power Factor)
*[GIS]: ガス絶縁開閉装置 (Gas Insulated Switchgear)
*[CB]: 遮断器 (Circuit Breaker)
*[DS]: 断路器 (Disconnecting Switch)
*[CT]: 変流器 (Current Transformer)
*[PT]: 計器用変圧器 (Potential Transformer)
*[VT]: 電圧変成器 (Voltage Transformer)
*[OCR]: 過電流継電器 (Over Current Relay)
*[OVR]: 過電圧継電器 (Over Voltage Relay)
*[GR]: 地絡継電器 (Ground Relay)
*[DGR]: 地絡方向継電器 (Directional Ground Relay)
*[OCGR]: 地絡過電流継電器 (Over Current Ground Relay)
*[ZCT]: 零相変流器 (Zero-phase-sequence Current Transformer)
*[EVT]: 接地形計器用変圧器 (Earthed Voltage Transformer)
*[ZPD]: 零相電圧検出器 (Zero-phase-sequence Potential Device)
*[VCB]: 真空遮断器 (Vacuum Circuit Breaker)
*[VCT]: 計器用変圧変流器 (combined Voltage and Current Transformer)
*[SOG]: 過電流蓄勢トリップ付地絡トリップ動作 (Storage Over current Ground)
*[AVR]: 自動電圧調整器 (Automatic Voltage Regulator)
*[STATCOM]: 静止型無効電力補償装置
*[SVC]: 静止型無効電力補償装置 (Static Var Compensator)
*[XLPE]: 架橋ポリエチレン絶縁 (Cross-Linked Polyethylene)
*[OF]: 油浸紙絶縁 (Oil-Filled)
*[ACSR]: 鋼心アルミより線 (Aluminum Conductor Steel Reinforced)
*[OPGW]: 光ファイバ複合架空地線
*[PAS]: 柱上気中開閉器 (Pole Air Switch)
*[LBS]: 高圧交流負荷開閉器 (Load Break Switch)
*[UPS]: 無停電電源装置 (Uninterruptible Power Supply)
*[SCADA]: 監視制御システム (Supervisory Control And Data Acquisition)
*[EMS]: エネルギー管理システム (Energy Management System)
*[PWM]: パルス幅変調 (Pulse Width Modulation)
*[FIT]: 固定価格買取制度 (Feed-in Tariff)
*[PV]: 太陽光発電 (Photovoltaics)
*[MPPT]: 最大電力点追従制御 (Maximum Power Point Tracking)
*[SOC]: 蓄電池充電率 (State of Charge)
*[η]: 効率 (Efficiency)
*[cosφ]: 力率 (Power Factor)
*[tanδ]: 誘電正接 (Dielectric Loss Tangent)
